...
class  Media {
 ...
 public function process_factura($descripcion,$nro_factura,$prov,$fecha_factura){
    if(!empty($this->errors)){
        return false;
      }
    if(empty($this->fileName) || empty($this->fileTempPath)){
    ...
    if(move_uploaded_file($this->fileTempPath,$this->productPath.'/'.$this->fileName))
    {
      ...
      if($this->insert_factura($descripcion,$nro_factura,$prov,$fecha_factura)){
        unset($this->fileTempPath);
        return true;
      }
...
/*--------------------------------------------------------------*/
  ...
  private function insert_factura($descripcion,$nro_factura,$prov,$fecha_factura){
        ...
        $query .=") VALUES (";
        $query .=" '{$prov}', '{$nro_factura}','{$descripcion}','{$fecha_factura}', '{$db->escape($this->fileName)}'";
        $query .=")";
...
/*--------------------------------------------------------------*/
   public function media_destroy($id,$file_name){
     ...
     if(delete_by_id_factura('factura',$id)){
